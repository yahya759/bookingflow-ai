...
function StaffPage() {
  ...
  const save = async () => {
    ...
    toast.success(editing ? "تم تحديث الطبيب" : "تم إضافة الطبيب");
    setOpen(false); load();
  };
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">أطباء العيادة.</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button onClick={openNew} className="bg-accent text-accent-foreground"><Plus className="h-4 w-4 ml-1"/>إضافة طبيب</Button></DialogTrigger>
          <DialogContent>
            <DialogHeader><DialogTitle>{editing ? "تعديل الطبيب" : "طبيب جديد"}</DialogTitle></DialogHeader>
            <div className="space-y-4">
              ...
              <div>
                <Label>أنواع الكشف المسندة</Label>
                <div className="mt-2 max-h-40 space-y-2 overflow-auto rounded-lg border border-border/60 p-3">
                  {services.length === 0 ? <p className="text-xs text-muted-foreground">أضف أنواع الكشف أولاً.</p> :
                    services.map((sv) => (
      ...
      {staff.length === 0 ? (
        <Card className="p-12 text-center">
          ...
          <p className="mt-3 text-sm text-muted-foreground">لا يوجد أطباء بعد.</p>
        </Card>
      ) : (
            ...
            <Card key={s.id} className="p-5">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">{(linked[s.id] ?? []).length} نوع كشف</p>
                </div>
