...
export const Route = createFileRoute("/login")({
  head: () => ({ meta: [{ title: "تسجيل الدخول — بوكلي" }] }),
  component: LoginPage,
});
...
function LoginPage() {
  ...
  const submit = async (e: FormEvent) => {
    ...
    else { toast.success("مرحباً بعودتك"); nav({ to: "/dashboard" }); }
  };
    ...
    <div className="relative min-h-screen bg-background">
      ...
      <div className="relative mx-auto flex min-h-screen max-w-md flex-col justify-center px-6 py-12">
        ...
        <Card className="glass p-8 shadow-elegant">
          ...
          <p className="mt-1 text-sm text-muted-foreground">سجّل دخولك للوحة التحكم.</p>
          ...
          <p className="mt-6 text-center text-sm text-muted-foreground">
            ليس لديك حساب؟ <Link to="/signup" className="text-accent hover:underline">أنشئ حساباً</Link>
          </p>
        </Card>
      </div>
    </div>
